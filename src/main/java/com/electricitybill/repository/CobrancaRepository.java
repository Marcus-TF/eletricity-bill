...
public class CobrancaRepository extends ConectDataBase {
    ...
    public CobrancaEntity findById(int id) {
        ...
        try (PreparedStatement preparedStatement = prepararSQL(FIND_BY_ID)) {
            ...
            while (rs.next()) {
                ...
                entity = new CobrancaEntity(id, mes_referencia, ano_referencia, tarifa_id, medicao_id);
                return entity;
            }
        } catch (SQLException e) {
            printSQLException(e);
    ...
    public List<CobrancaEntity> findAll() {
        List<CobrancaEntity> entityList = new ArrayList<>();
        try (PreparedStatement preparedStatement = prepararSQL(FIND_ALL)) {
            ...
            while (rs.next()) {
                ...
                int tarifa_id = rs.getInt("tarifa_id");
                int medicao_id = rs.getInt("medicao_id");
                entityList.add(new CobrancaEntity(id, mes_referencia, ano_referencia, tarifa_id, medicao_id));
            }
        } catch (SQLException e) {
            printSQLException(e);
        } catch (ClassNotFoundException e) {
