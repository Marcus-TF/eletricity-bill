...
public class TimeRotaRepository extends ConectDataBase {
    ...
    public TimeRotaEntity findById(int id) {
        ...
        try (PreparedStatement preparedStatement = prepararSQL(FIND_BY_ID)) {
            ...
            while (rs.next()) {
                ...
                entity = new TimeRotaEntity(id, funcionario_id, tarefa_rota_id);
                return entity;
            }
        } catch (SQLException e) {
            printSQLException(e);
    ...
    public List<TimeRotaEntity> findAll() {
        List<TimeRotaEntity> entityList = new ArrayList<>();
        try (PreparedStatement preparedStatement = prepararSQL(FIND_ALL)) {
            ...
            while (rs.next()) {
                ...
                int funcionario_id = rs.getInt("funcionario_id");
                int tarefa_rota_id = rs.getInt("tarefa_rota_id");
                entityList.add(new TimeRotaEntity(id, funcionario_id, tarefa_rota_id));
            }
        } catch (SQLException e) {
            printSQLException(e);
        } catch (ClassNotFoundException e) {
