...
public class TarifaRepository extends ConectDataBase {
    ...
    public TarifaEntity findById(int id) {
        ...
        try (PreparedStatement preparedStatement = prepararSQL(FIND_BY_ID)) {
            ...
            while (rs.next()) {
                ...
                entity = new TarifaEntity(id, taxa, classe_id, lei, data_inicio, data_fim, aliquota_ICMS);
                return entity;
            }
        } catch (SQLException e) {
            printSQLException(e);
    ...
    public List<TarifaEntity> findAll() {
        List<TarifaEntity> entityList = new ArrayList<>();
        try (PreparedStatement preparedStatement = prepararSQL(FIND_ALL)) {
            ...
            while (rs.next()) {
                ...
                Date data_fim = rs.getDate("data_fim");
                String aliquota_ICMS = rs.getString("aliquota_ICMS");
                entityList.add(new TarifaEntity(id, taxa, classe_id, lei, data_inicio, data_fim, aliquota_ICMS));
            }
        } catch (SQLException e) {
            printSQLException(e);
        } catch (ClassNotFoundException e) {
