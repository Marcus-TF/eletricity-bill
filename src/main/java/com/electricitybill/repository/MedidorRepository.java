...
public class MedidorRepository extends ConectDataBase {
    ...
    public MedidorEntity findById(int id) {
        ...
        try (PreparedStatement preparedStatement = prepararSQL(FIND_BY_ID)) {
            ...
            while (rs.next()) {
                ...
                entity = new MedidorEntity(id, descricao, id_poste, id_rota);
                return entity;
            }
        } catch (SQLException e) {
            printSQLException(e);
    ...
    public List<MedidorEntity> findAll() {
        List<MedidorEntity> entityList = new ArrayList<>();
        try (PreparedStatement preparedStatement = prepararSQL(FIND_ALL)) {
            ...
            while (rs.next()) {
                ...
                int id_poste = rs.getInt("id_poste");
                int id_rota = rs.getInt("id_rota");
                entityList.add(new MedidorEntity(id, descricao, id_poste, id_rota));
            }
        } catch (SQLException e) {
            printSQLException(e);
        } catch (ClassNotFoundException e) {
