...
public class TipoPessoaRepository extends ConectDataBase {
    ...
    public TipoPessoaEntity findById(int id) {
        TipoPessoaEntity tipoPessoaEntity = null;
        try (PreparedStatement preparedStatement = prepararSQL(FIND_BY_ID)) {
            ...
            while (rs.next()) {
                ...
                tipoPessoaEntity = new TipoPessoaEntity(id, descricao);
    ...
    public List<TipoPessoaEntity> findAll() {
        List<TipoPessoaEntity> tipoPessoaEntityList = new ArrayList<>();
        try (PreparedStatement preparedStatement = prepararSQL(FIND_ALL)) {
            ...
            while (rs.next()) {
                int id = rs.getInt("id");
                String descricao = rs.getString("descricao");
                tipoPessoaEntityList.add(new TipoPessoaEntity(id, descricao));
            }
        } catch (SQLException e) {
            printSQLException(e);
        } catch (ClassNotFoundException e) {
