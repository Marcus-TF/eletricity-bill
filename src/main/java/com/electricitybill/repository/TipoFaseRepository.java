...
public class TipoFaseRepository extends ConectDataBase {
    ...
    public TipoFaseEntity findById(int id) {
        ...
        try (PreparedStatement preparedStatement = prepararSQL(FIND_BY_ID)) {
            ...
            while (rs.next()) {
                ...
                entity = new TipoFaseEntity(id, descricao);
                return entity;
            }
        } catch (SQLException e) {
            printSQLException(e);
    ...
    public List<TipoFaseEntity> findAll() {
        List<TipoFaseEntity> entityList = new ArrayList<>();
        try (PreparedStatement preparedStatement = prepararSQL(FIND_ALL)) {
            ...
            while (rs.next()) {
                int id = rs.getInt("id");
                String descricao = rs.getString("descricao");
                entityList.add(new TipoFaseEntity(id, descricao));
            }
        } catch (SQLException e) {
            printSQLException(e);
        } catch (ClassNotFoundException e) {
