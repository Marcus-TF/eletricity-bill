...
public class PosteRepository extends ConectDataBase {
    ...
    public PosteEntity findById(int id) {
        ...
        try (PreparedStatement preparedStatement = prepararSQL(FIND_BY_ID)) {
            ...
            while (rs.next()) {
                ...
                entity = new PosteEntity(id, latitude, longitude, codigo, observacao);
                return entity;
            }
        } catch (SQLException e) {
            printSQLException(e);
    ...
    public List<PosteEntity> findAll() {
        List<PosteEntity> entityList = new ArrayList<>();
        try (PreparedStatement preparedStatement = prepararSQL(FIND_ALL)) {
            ...
            while (rs.next()) {
                ...
                String codigo = rs.getString("codigo");
                String observacao = rs.getString("observacao");
                entityList.add(new PosteEntity(id, latitude, longitude, codigo, observacao));
            }
        } catch (SQLException e) {
            printSQLException(e);
        } catch (ClassNotFoundException e) {
