...
public class TarefaRotaRepository extends ConectDataBase {
    ...
    public TarefaRotaEntity findById(int id) {
        ...
        try (PreparedStatement preparedStatement = prepararSQL(FIND_BY_ID)) {
            ...
            while (rs.next()) {
                ...
                entity = new TarefaRotaEntity(id, observacao, data_inicio, data_fim, rota_id);
                return entity;
            }
        } catch (SQLException e) {
            printSQLException(e);
    ...
    public List<TarefaRotaEntity> findAll() {
        List<TarefaRotaEntity> entityList = new ArrayList<>();
        try (PreparedStatement preparedStatement = prepararSQL(FIND_ALL)) {
            ...
            while (rs.next()) {
                ...
                Date data_fim = rs.getDate("data_fim");
                int rota_id = rs.getInt("rota_id");
                entityList.add(new TarefaRotaEntity(id, observacao, data_inicio, data_fim, rota_id));
            }
        } catch (SQLException e) {
            printSQLException(e);
        } catch (ClassNotFoundException e) {
